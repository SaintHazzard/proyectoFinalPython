...
class RutaEntrenamiento:
  def __init__(self, nombres = None, modulos = [], sgdbPrincipal=None, sgdbAlternativo=None):
  ...
  def crearRuta(cls,nombreRuta):
      ...
      while True:
        try:
          ...
          if 1 <= nRuta <= len(lenElegir)+1 or nRuta==5:
            return nRuta
          else: print("Opcion invalida. Por favor ingrese un numero valido")
    ...
    MODULOS = {
          ...
          # "bd": Modulo("Bases de datos", ["MySQL", "MongoDB", "PostgreSQL"]).getModulo(),
      }
    ruta = cls(nombreRuta);crearJson(ruta,f"jsonDataRutas/{ruta.nombres}.json")
    ...
    while True:
      ...
      if  moduloSeleccionado in MODULOS:
        ...
        if MODULOS[moduloSeleccionado] not in ruta.modulos:
          ruta.modulos.append(MODULOS[moduloSeleccionado])
          MODULOS.pop(moduloSeleccionado)
          MODULOSNAME.remove(moduloSeleccionado)
          print(f'\t\tModulo {moduloSeleccionado.capitalize()} agregado')
        else: print(f'{"-"*20}El modulo ya esta en la ruta{"-"*20}')
        input('Presione enter para continuar')
      if nRuta == 5:
        dbs = ["MySQL", "MongoDB", "PostgreSQL"]
        while True:
          return seleccionarDbs(ruta,dbs)
